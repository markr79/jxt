...
module.exports = function(JXT, opts) {
  function Stanza(data, xml, parent) {
    ...
    var parentNS = (parentNode || {}).namespaceURI || (parentNode ? JXT.getNamespace(parentNode.name) : undefined);

    if (!parentNS) {
      parentNS = parentNode ? JXT.getNamespace(parentNode.name) : undefined;
    }
    ...
    for (var i = 0, len = self.xml.childNodes.length; i < len; i++) {
      ...
      var ChildJXT = JXT.getDefinition(child.localName, child.namespaceURI, false, true);
